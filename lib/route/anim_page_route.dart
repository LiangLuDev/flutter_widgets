...
class AnimPageRoute extends PageRouteBuilder {
  final Widget widget;

  AnimPageRoute(this.widget)
      : super(
            opaque: false,
...
class PageEntryState extends State<PageEntry> with SingleTickerProviderStateMixin {
  ...
  void changeOffset(DragUpdateDetails details, {bool isSlideClose = false}) {
    ...
    if (isSlideClose) {
      ...
      closePosition = size.height * 0.1;
      canClosePosition = offsetPositionDy;
    } else {
      percent = details.localPosition.dx / size.width;
      offset = Offset(percent, 0.0);
  ...
  void logout({bool isSlideClose = false}) {
    Tween<Offset> tween;
    if (isSlideClose) {
      tween = Tween<Offset>(begin: Offset(0.0, this.positionPercent), end: Offset(0.0, -1));
    } else {
      tween = Tween<Offset>(begin: Offset(this.positionPercent, 0.0), end: Offset(1, 0.0));
    }
    this.position = tween.animate(CurvedAnimation(parent: controller, curve: Curves.fastOutSlowIn));
