...
class CustomScrollController extends ScrollController {
  ...
  /// 则创建并附加到此控制器的新 [CustomScrollPosition] 对象的偏移量将初始化为该值。
  /// 默认为 0.0。
  @override
  ...
  ///
  /// 不应直接突变。
  /// 可以使用 [attach] 和 [detach] 添加和删除 [CustomScrollPosition] 对象。
  @protected
  @override
  Iterable<CustomScrollPosition> get positions => _positions;
  final List<CustomScrollPosition> _positions = <CustomScrollPosition>[];

  /// 是否有任何 [CustomScrollPosition] 对象已使用 [attach] 方法
  /// 将自身附加到 [ScrollController]。
  ///
  /// 如果为 false，则不得调用与 [CustomScrollPosition] 交互的成员，
  ...
  /// 动画都会中断。如果 [CustomScrollPosition] 不会过度滚动，而是允许滚动超出范围，
  /// 那么超出范围不会中断动画。
  ///
  /// 动画对视口或内容尺寸的更改无动于衷。
  ///
  ...
  @override
  void attach(covariant CustomScrollPosition position) {
  ...
  /// 创建一个 [CustomScrollPosition] 供 [Scrollable] 小部件使用。
  ///
  /// 子类可以重写此功能，以自定义其控制的可滚动小部件使用的 [CustomScrollPosition]。
